const settings = input.config({
  ...
  description:
    "Select a table and have an LLM (OpenAI) create your field descriptions for you. It will pull a handful of records from the base and use those as a reference for what data is in the fields. Scripting doesn’t let us examine formula code. Requires an OpenAI API Key and uses gpt-4.1-mini (you can change in the source code)",
  items: [
    ...
    input.config.select("all_fields", {
      ...
      description:
        "If yes, will update for all fields even if they have a description. If no, only do this for net new fields. Default is only net new",
      options: [
